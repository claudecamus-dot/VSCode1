...
... EOF`. Two passes avoid that: strip heredoc bodies first (they're always
data, never a command to execute), then only match `git push`/`git reset` at
the START of a shell segment (splitting on &&, ||, ;, |, newline — but not
