...
(helpers + géométrie), `restitution-ppt` (structure et invariants du deck de CE
projet), `deck-design-review` (revue contrat par slide de CE deck).
...
3. **Transposer, pas copier** : adapter les cotes au gabarit du deck cible
   (ce projet : 10×5.625in OCTO — `template ppt/template.pptx`, helpers
   `app/scripts/pptx_deck.py` — `add_card`, `add_rect`, `add_hbar`, `add_gauge`,
   `add_polygon`, `add_range_bar`, `estimer_lignes`…) en respectant sa charte
   (pas d'ombres, police du thème, `verifier_geometrie` +
   `verifier_debordements_texte` à zéro). Un composant du catalogue absent de la
   lib locale (badge, chip, teardrop, encart) existe probablement déjà dans la lib
   du projet frère (`VSCode2/app/services/pptx_deck.py`) — le porter plutôt que
   le redessiner.
