...
Conception : `docs/reflexions/agent-superviseur.md` (doc de référence **portée depuis le
projet source** — Interview-to-Deck/VSCode2, importée le 2026-07-21, voir son en-tête).
L'étage 1 (scan déterministe, 0 token — `.claude/supervision/scan_transcripts.py`) mesure ;
cet étage **qualifie** — et l'humain tranche. Sortie unique :
...
- **Jamais les JSONL bruts** : ne pas ouvrir `~/.claude/projects/.../*.jsonl` ni
  ...
  peuvent contenir du contenu de sessions client (réponses au questionnaire, commentaires
  d'équipe) : analyse strictement locale.
...
- `MEMORY.md` (l'index seulement) : les leçons `feedback_*`/`reference_*` déjà
  capitalisées — ne pas re-diagnostiquer ce qui y est déjà, citer la mémoire comme preuve
  si un constat la confirme sur données récentes.
...
| `verification-manquante` | Quelle vérification réelle est systématiquement sautée ? | `verifications_oubliees` ; commit touchant `app/scripts/pptx_deck.py`/`export-restitution-ppt.py` sans passage `pptx-verify` |
...
`bmad-customize`, amender un playbook — décisions à consigner dans
`.claude/supervision/arbitrages.json`). Une leçon durable sur la façon de travailler →
