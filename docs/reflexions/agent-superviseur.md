> **Doc de référence importée** — ce fichier documente la conception et l'historique de
> construction de la skill `agent-supervisor` sur le **projet source** (Interview-to-Deck,
> `VSCode2`, stack FastAPI/Jinja2/HTMX), porté ici le **2026-07-21** avec le duo
> `agent-orchestrator`/`agent-supervisor` (cf. `export/README.md` du projet source,
> § « Comment reprendre dans un autre projet »). Les dates, métriques d'exemple (POC du
> 2026-07-17, invocations `run-dev-server` ×9…) et arbitrages cités ci-dessous décrivent
> **le projet source**, pas ce projet-ci — l'architecture en 2 étages et le raisonnement
> restent valables tels quels, c'est pour cette raison qu'ils sont conservés en référence.
> Les **données réelles de ce projet-ci** vivent dans `.claude/supervision/` (générées,
> gitignorées sauf `arbitrages.json` — vide à l'import, rien n'a encore été arbitré ici) et
> dans `docs/wiki/technical/agents-supervision.md` (tableau de bord régénéré à chaque
> session) — à consulter en priorité pour tout diagnostic concret sur ce projet.
>
> ---
...
*(Sur ce projet-ci, le premier scan réel du 2026-07-21 a produit un résultat directement
comparable — cf. `docs/wiki/technical/agents-supervision.md` : `revue-increment` également
jamais invoquée comme skill, 46 skills BMAD également à 0 invocation ; en revanche la
génération PPT est passée par le sous-agent projet `ppt-designer`, pas par le skill global
`pptx-deck`, une différence de canal propre à ce projet.)*
