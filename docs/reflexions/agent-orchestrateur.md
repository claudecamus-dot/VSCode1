...
> construction de la skill `agent-orchestrator` sur le **projet source**
> (Interview-to-Deck, `VSCode2`, stack FastAPI/Jinja2/HTMX), porté ici le **2026-07-21**
> avec le duo `agent-orchestrator`/`agent-supervisor` (cf. `export/README.md` du projet
> source, § « Comment reprendre dans un autre projet »). Les dates, précédents concrets
> (US9.12, `pptx_export.py`, `record_libre.html`…) et statuts cités ci-dessous décrivent
> **le projet source**, pas ce projet-ci (questionnaire de maturité agile/produit,
